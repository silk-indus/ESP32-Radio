...
// ESP32dev Signal  Wired to LCD        Wired to       SDCARD   Wired to the rest
// -------- ------  --------------      -------------------  ------   ---------------
// GPIO32           -                   pin 1 XDCS            -       -
// GPIO5            -                   pin 2 XCS             -       -
// GPIO4            -                   pin 4 DREQ            -       -
...
char*     prevod_ip ( char* s ) ;            // In place conversion full string
void      dsp_frame( char * s, int x, int y, int wx, int wy);
...
bool              ce = false;                            // Central european stream info
...
char              p_station[50];                         // Station name from preferences
String            p_numst = "";                          // Station number
...
int ir_nums[] = {0x4ab5,0x6897,0x9867,0xb04f,0x30cf,0x18e7,0x7a85,0x10ef,0x38c7,0x5aa5};
...
void dsp_frame( char * s, int x, int y, int wx, int wy){
  Serial.println(tft.fontHeight(tft.fontsLoaded()));
  int dy = 0, i=0;
  String ss = "";
  while(i<strlen(s)){
    while (i<strlen(s) && tft.textWidth(ss) < wx && x + tft.textWidth(ss+s[i]) < tft.width()) {
      i++;
      if ( s[i-1] != '\n') ss += s[i-1];
      else break;
    }
    tft.drawString( ss, x, y + dy);
    Serial.printf("y+dy: %d+%d ",y,dy);
    Serial.println(ss);
    ss="";
    if (dy + tft.fontHeight(tft.fontsLoaded()) < wy && (y + dy + tft.fontHeight(tft.fontsLoaded())) < tft.height()) dy += tft.fontHeight(tft.fontsLoaded());
    else break;
  }
}

char* prevod_ip ( char* s ){
  const char cp1250[] = {0x8a, 0x8d, 0x8e, 0x9a, 0x9d, 0x9e, 0xbc, 0xbe, 0xc0, 0xc1, 0xc4, 0xc5, 0xc8, 0xc9, 0xcc, 0xcd, 0xcf, 0xd2, 0xd3, 0xd4, 0xd8, 0xd9, 0xda, 0xdd, 0xe0, 0xe1, 0xe4, 0xe5, 0xe8, 0xe9, 0xec, 0xed, 0xef, 0xf2, 0xf3, 0xf4, 0xf8, 0xf9, 0xfa, 0xfd};
//const char cp1250[] = "\0x8a \0x8d \0x8e \0x9a \0x9d \0x9e \0xbc \0xbe \0xc0 \0xc1 \0xc4 \0xc5 \0xc8 \0xc9 \0xcc \0xcd \0xcf \0xd2 \0xd3 \0xd4 \0xd8 \0xd9 \0xda \0xdd \0xe0 \0xe1 \0xe4 \0xe5 \0xe8 \0xe9 \0xec \0xed \0xef \0xf2 \0xf3 \0xf4 \0xf8 \0xf9 \0xfa \0xfd ";
  const char   utf8[] = "\u0160\u0164\u017d\u0161\u0165\u017e\u013d\u013e\u0154\u00c1\u00c4\u0139\u010c\u00c9\u011a\u00cd\u010e\u0147\u00d3\u00d4\u0158\u016e\u00da\u00dd\u0155\u00e1\u00e4\u013a\u010d\u00e9\u011b\u00ed\u010f\u0148\u00f3\u00f4\u0159\u016f\u00fa\u00fd";
//const char  utf8l[] = "\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5";
//const char  utf8h[] = "\0x87\0x87\0x87\0x87\0x87\0xbe\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5\0xc5";
  
   
  char * e = (char *) malloc (150);
  char subh, subl;
  uint8_t count = 0;
  //for(int j = 0; j<strlen(cp1250); j++) Serial.printf("[%x]",cp1250[j]);
  for (int i = 0 ; i<strlen(s)/sizeof(s[0]) ; i++ )          // For every input character
  {
    subl = ' ';
    for(int j = 0; j<40; j++){
      if(s[i] == cp1250[j]) {
        //Serial.printf("[%x]-%d,%d ",s[i],i,j);
        subl=utf8[2*j];
        subh=utf8[2*j+1];
      }
    }
  ...
  e[count] = 0;
  //for(int i = 0 ; i< (strlen(e)<150 ? strlen(e) : 150) ; i++ ) s[i] = e[i];
  return e;
}

/*
void prevod ( char *ascii1 , char *ascii2 )
{
  const char CS[] = "ÁÄÉÍÓÔÚÝáäéíóôúýČčĎďĹĺĽľŇňŔŕŘřŠšŤťŮůŽž";
  const char CSr[] = "\u0080\u0081\u0082\u0083\u0084\u0085\u0086\u0087\u0088\u0089\u008a\u008b\u008c\u008d\u008e\u008f\u0090\u0091\u0092\u0093\u0094\u0095\u0096\u0097\u0098\u0099\u009a\u009b\u009c\u009d\u009e\u009f\u00a0\u00a1\u00a2\u00a3\u00a4\u00a5";

  for (int i = 0; i< strlen(CS)-1; i+=2){
    if((CS[i] & 0xE0) == 0xC0){
      if((CS[i] == (*ascii1)) && (CS[i+1] == (*ascii2))) {
        *ascii1 = CSr[i];
        *ascii2 = CSr[i+1];
        Serial.printf("%x %x -> %x %x\n",CS[i],CS[i+1],CSr[i],CSr[i+1]);
      }
    }
  }
}

void prevod_ip ( char* s )
{

  for (int i = 0 ; i<strlen(s)-1 ; i++ )          // For every input character
  {
    Serial.printf("%c[%x] ",s[i],s[i]);

    prevod ( &s[i], &s[i+1] ) ;          // Translate if necessary
...
void showstreamtitle ( const char *ml, bool full )
{
  ...
  if ( strstr ( ml, "StreamTitle=" ) )
  {
    ...
    if(ce) {
      //streamtitle[2] = 0xcf;
      char * p = prevod_ip(streamtitle);
      strcpy( streamtitle, p );
      free (p);
    }
  }
  else if ( full )
...
bool connecttohost()
{
  ...
  tftset ( 2, p_station);                           // print station name from preferences
  tftset ( 4, String(ini_block.newpreset).c_str()); // print station number
...
void scanIR()
{
  ...
  if ( ir_value )                                           // Any input?
  {
    ...
    {
      if(p_numst.length() <3){
      if (ir_value == ir_nums[1]) p_numst += '1';
      if (ir_value == 0x9867) p_numst += '2';
      if (ir_value == 0xb04f) p_numst += '3';
      if (ir_value == 0x30cf) p_numst += '4';
      if (ir_value == 0x18e7) p_numst += '5';
      if (ir_value == 0x7a85) p_numst += '6';
      if (ir_value == 0x10ef) p_numst += '7';
      if (ir_value == 0x38c7) p_numst += '8';
      if (ir_value == 0x5aa5) p_numst += '9';
      if (ir_value == 0x4ab5) p_numst += '0';
      }
...
void getsettings()
{
  ...
  for ( i = 0 ; i < MAXPRESETS ; i++ )                   // Max number of presets
  {
    ...
    if ( statstr != "" )                                 // Preset available?
    {
      ...
      { 
        statstr.remove ( 0, inx + 1 + ce ) ;                  // Yes, remove non-comment part
      }
...
void setup()
{
  ...
  if (SPIFFS.exists("/arialbd8.vlw")    == false) font_missing = true;
  if (SPIFFS.exists("/arialbd11.vlw")    == false) font_missing = true;
  if (SPIFFS.exists("/arialbd16.vlw")    == false) font_missing = true;
  ...
  //VS1053 vs1053player(ini_block.vs_cs_pin, ini_block.vs_dcs_pin, ini_block.vs_dreq_pin);
       ...
       ( ini_block.tft_scl_pin >= 0 ) )
  {
    ...
    {
      Serial.println("Disp initialized");
      //dsp_setRotation(1) ;                                // Use landscape format
      ...
      //dsp_println ( tmpstr ) ;
      //dsp_setTextSize ( 1 ) ;                            // Bigger character font
      //dsp_setFont();
      //dsp_setCursor ( 0, 38 ) ;
      //dsp_println ( " !+ľščťžýáíéĽôŤŇň" ) ;

      tft.setRotation(1);
      tft.fillScreen(tft.textbgcolor);
      tft.setCursor(0,10);
      tft.print( "Starting...\nVersion:") ;
      //tft.print( tmpstr ) ;

       tft.loadFont(FONT_MEDIUM); // Must load the font first
       tft.print("\nEd Smallenburg feat. SILK\n");
       //tft.print("ÁÄÉÍÓÔÚÝáäéíóôúýČčĎďĹĺĽľŇňŔŕŘřŠšŤťŮůŽž\n");
       //dsp_frame( "ÁÄÉÍÓÔÚÝáäéíóôúýČčĎďĹĺĽľŇňŔŕŘřŠšŤťŮůŽž\n", 20, 80, 80, 40);
       tft.unloadFont(); 
...
void mp3loop()
{
  ...
  if ( ini_block.newpreset != currentpreset )            // New station or next from playlist requested?
  {
    ...
    else
    {
      ...
      {
        ce = false;
        if (host[inx+1] == '.') ce=true;                  // '.' in station name means cp1250 coding
        if (ce) host.substring(inx+2).toCharArray(p_station, host.length() + 1);  // get station name from preferences
        else host.substring(inx+1).toCharArray(p_station, host.length() + 1); 
      }
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  {
    //xxx vs1053player->AdjustRate ( ivalue ) ;             // Yes, adjust
  }
...
void displayinfo ( uint16_t inx )
{
  ...
  if ( true )                                               // TFT active?
  {
    dsp_fillRect ( p->x, p->y, (p->width ==0 ? width : p->width), p->height, BLACK ) ;    // Clear the space for new info
    if ( ( dsp_getheight() > 64 ) && ( p->y > 1 ) )        // Need and space for divider?
    {
      dsp_fillRect ( 0, p->y - 4, tft.width(), 1, GREEN ) ;      // Yes, show divider above text
    }
    ...
    if ( len++ )                                           // Check string length, set buffer length
    {
      ...
      tft.setTextColor(p->color, BLACK);
      switch(inx){
        case 0:
          dsp_setTextSize ( 1 ) ;
          tft.setTextFont(1);
        ...
        case 2:
          ...
          break;
        case 4:
          tft.loadFont(FONT_NUMS);
          break;
      }
      
      dsp_frame( buf, p->x, p->y, (p->width ==0 ? width : p->width), p->height);
      tft.unloadFont();
    ...
    if ( xQueueReceive ( dataqueue, &inchunk, 5 ) )
    {
      ...
      switch ( inchunk.datatyp )                                    // What kind of chunk?
      {
        ...
        case QSTOPSONG:
          ...
          vs1053player.setVolume ( 70 ) ;                           // Mute
